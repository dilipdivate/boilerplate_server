...
const getBookDetailByAuthor = catchAsync(async (req, res) => {
...
const getBookDetailByGenre = catchAsync(async (req, res) => {
  const book = await bookService.getBookByGenre(req.params.genre);
  if (!book) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Book not found');
  }
  res.send(book);

...
const getCustomerDetailByNumber = catchAsync(async (req, res) => {
  const customer = await customerService.getCustomerByUser(req.params.user);
  if (!customer) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Customer not found');
  }
  res.send(customer);
